...
template<int D>
void SPH<D>::calculate_Omega(Particle<D> *sinks, int nsinks, 
        Particle<D> *sources, int nsources) {
...
template<int D>
void SPH<D>::calculate_h(Particle<D> *sinks, int nsinks, Particle<D> *sources,
        int nsources) {

    bool not_converged = true;

    while (not_converged) {
        calculate_rho(sinks, nsinks, sources, nsources);
        calculate_Omega(sinks, nsinks, sources, nsources);
...
template<int D>
void SPH<D>::calculate_rho(Particle<D> *sinks, int nsinks, 
        Particle<D> *sources, int nsources) {
...
template<int D>
void SPH<D>::calculate_accel(Particle<D> *sinks, int nsinks, 
        Particle<D> *sources, int nsources) {
...
void SPH<D>::update_timestep() {
    ...
    Grid *G = new Grid(100, P, S->np);

    for (int i=0; i<G->ncells; i++) {
        Particle<D> *sinks = G->get_sinks(i, P);
        int nsinks = G->c[i].nparticles;
        Particle<D> *sources = G->get_sources(i, P);
        int nsources = G->c[i].nsources;

        for (int j=0; j<G->c[i].nparticles; j++) {
            double dv = del_dot_v(&sinks[j], sources, nsources);

            double v = sinks[j].h*fabs(dv)+sinks[j].cs+1.2*sinks[j].alpha*
                sinks[j].cs;
            if (dv < 0)
                v += S->beta*sinks[j].h*fabs(dv);

            double temp_dt = 0.25*sinks[j].h/v;
...
void SPH<D>::initialize() {
    ...
    for (int i=0; i<G->ncells; i++) {
        ...
        calculate_h(sinks, nsinks, sources, nsources);
        calculate_rho(sinks, nsinks, sources, nsources);
        calculate_Omega(sinks, nsinks, sources, nsources);

        G->update_sinks(i, P, sinks);
        delete sources;
    }
    calculate_P();
    calculate_cs();
...
void SPH<D>::step() {
    ...
    for (int i=0; i<G->ncells; i++) {
        ...
        calculate_accel(sinks, nsinks, sources, nsources);
        calculate_diss(sinks, nsinks, sources, nsources);
    ...
    for (int i=0; i<G->ncells; i++) {
        ...
        calculate_uvel(sinks, nsinks, sources, nsources);
        calculate_udiss(sinks, nsinks, sources, nsources);
        calculate_alphavel(sinks, nsinks, sources, nsources);
        calculate_alphauvel(sinks, nsinks, sources, nsources);
